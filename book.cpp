...
int checksite(string check) /*Перменная возвращает 1, если массив проходит проверку;
							возвращает 0, если не проходит*/
{
	for (int i = 0; i <= k; i++) {
		if (sitename[i] == check) {
			return 0;
...
{
	string check = ""; /// check принимает данные для проверки наличия такой же строки в массиве sitename  
	cout << "Enter website: ";
	cin >> check;
...
void print(int c)
/* Функция для отображения данных, введеных пользователем, в консоли. Принимает значение индекса c массива*/ {
	if (sitename[c] == "")
		return;
	if (k > -1) {
...
void updtinfo(int c)
/*Функция обновляет данные, введенные пользователем, принимает переменную индекс массива.  */
{
	string check = "";
	cout << "Enter website: ";
	cin >> check;
...
void delSite(int c)
/* Функция для удаления веб - сайта, добавленного пользователем. Принимает переменную индекс массива */
{
	sitename[c] = "";
	number[c] = "";
	email[c] = "";
